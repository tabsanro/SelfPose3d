...
class CampusSynthetic(Dataset):
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        self.gt_3d_file = cfg.DATASET.GT_3D_FILE
        ...
        pose_db_file = os.path.join(self.dataset_root, "..", self.gt_3d_file)
    ...
    def __getitem__(self, idx):
        ...
        for n in range(0, nposes):
            ...
            new_xy = (
                rotate_points(points, center, rot_rad) - center + new_center
            )
    ...
    def _get_single_view_item(self, joints_3d, joints_3d_vis, cam):
        ...
        meta = {
            ...
            if self.train_pseudo_gt3d
            else (joints_3d_u[:, 11] + joints_3d_u[:, 12]) / 2.0,
    ...
    def compute_human_scale(pose, joints_vis):
        ...
        return np.clip(
            np.maximum(maxy - miny, maxx - minx) ** 2,
            1.0 / 4 * 96 ** 2,
            4 * 96 ** 2,
        )
        ...
        if self.target_type == "gaussian":
            ...
            for n in range(nposes):
                ...
                for joint_id in range(num_joints):
                    ...
                    g_x = (
                        max(0, -ul[0]),
                        min(br[0], self.heatmap_size[0]) - ul[0],
                    )
                    g_y = (
                        max(0, -ul[1]),
                        min(br[1], self.heatmap_size[1]) - ul[1],
                    )
                    # Image range
    ...
    def generate_3d_target(self, joints_3d):
        ...
        target = np.zeros(
            (cube_size[0], cube_size[1], cube_size[2]), dtype=np.float32
        )
        cur_sigma = 200.0
        ...
        for n in range(num_people):
            if self.train_pseudo_gt3d:
                ...
                mu_x = (
                    joints_3d[n][joint_id[0]][0] + joints_3d[n][joint_id[1]][0]
                ) / 2.0
                mu_y = (
                    joints_3d[n][joint_id[0]][1] + joints_3d[n][joint_id[1]][1]
                ) / 2.0
                mu_z = (
                    joints_3d[n][joint_id[0]][2] + joints_3d[n][joint_id[1]][2]
                ) / 2.0
            ...
            target[
                i_x[0] : i_x[1], i_y[0] : i_y[1], i_z[0] : i_z[1]
            ] = np.maximum(
    ...
    def isvalid(self, new_center, bbox, bbox_list):
        ...
        for k, cam in self.cameras.items():
            ...
            if (
                10 < loc_2d[0, 0] < width - 10
                and 10 < loc_2d[0, 1] < height - 10
            ):
                vis += 1
