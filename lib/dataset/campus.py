...
class Campus(JointsDataset):
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        self.train_pseudo_gt3d = cfg.DATASET.TRAIN_PSEUDO_GT3D
    ...
    def _get_db(self):
        ...
        actor_3d = np.array(np.array(data['actor3D'].tolist()).tolist(), dtype="object").squeeze()  # num_person * num_frame
        ...
        for i in self.frame_range:
            for k, cam in cameras.items():
                ...
                if self.train_pseudo_gt3d:
                    preds = [self.coco2panoptic(p) for p in preds]
    ...
    def evaluate(self, preds, recall_threshold=500):
        ...
        for i, fi in enumerate(self.frame_range):
            if self.train_pseudo_gt3d:
                pred_panoptic = preds[i].copy()
                pred_panoptic = pred_panoptic[pred_panoptic[:, 0, 3] >= 0, :, :3]
                pred = np.stack([self.panotic2campus3D(p) for p in copy.deepcopy(pred_panoptic[:, :, :3])])
            else:
    ...
    @staticmethod
    def panotic2campus3D(panoptic_pose):
        """
        transform panotic order(our method output) 3d pose to shelf dataset order with interpolation
        :param panoptic_pose: np.array with shape 15x3
        :return: 3D pose in shelf order with shape 14x3
        """
        shelf_pose = np.zeros((14, 3))
        panoptic2shelf = np.array([14, 13, 12, 6, 7, 8, 11, 10, 9, 3, 4, 5])
        shelf_pose[0: 12] += panoptic_pose[panoptic2shelf]

        mid_sho = (panoptic_pose[3] + panoptic_pose[9]) / 2  # L and R shoulder
        head_center = panoptic_pose[1]  # nose
        ...
        shelf_pose[12] = (shelf_pose[8] + shelf_pose[9]) / 2  # Use middle of shoulder to init
        shelf_pose[13] = panoptic_pose[1]  # use nose to init

        shelf_pose[13] = shelf_pose[12] + (shelf_pose[13] - shelf_pose[12]) * np.array([0.75, 0.75, 1.5])
        shelf_pose[12] = shelf_pose[12] + (panoptic_pose[1] - shelf_pose[12]) * np.array([0.5, 0.5, 0.5])
    ...
    def coco2panoptic(coco_pose):
        """
        transform panotic order(our method output) 3d pose to shelf dataset order with interpolation
        :param panoptic_pose: np.array with shape 17x3
        :return: 3D pose in shelf order with shape 15x3
        """
        panoptic_pose = np.zeros((15, 3))
        mapping = [0, 0, 0, 5, 7, 9, 11, 13, 15, 6, 8, 10, 12, 14, 16]
        panoptic_pose = coco_pose[mapping]

        neck = (coco_pose[5] + coco_pose[6]) / 2.0
        hip = (coco_pose[11] + coco_pose[12]) / 2.0
